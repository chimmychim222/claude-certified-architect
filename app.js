...
{d:"Agentic Architecture & Orchestration",q:"An order-processing system crashes mid-run. Every subagent had exported its progress to a known location and the coordinator loads the manifest on restart, yet the respawned subagents work through items the manifest already records as finished. What is missing?",o:["The manifest has to be injected into each respawned subagent's prompt, because a fresh subagent begins with nothing the coordinator has read","The manifest should be written after every tool call rather than at task boundaries, so the record is fine grained enough to restart from","The coordinator should reload the manifest before each delegation rather than once at restart, so an export written late in the run is not missed","The crash left entries describing work whose effects never committed, so repeating them is the correct behaviour for a manifest that cannot be trusted"],a:0,
e:"Task Statement 5.4 describes crash recovery as structured state exports that the coordinator loads on resume and injects into agent prompts, and the injection is the half that is missing here. A respawned subagent is a new subagent: it does not inherit the coordinator's context, so the coordinator knowing what was finished changes nothing about what the subagent believes. Writing the manifest more often changes how much work is recorded, not whether the record reaches the agent that needs it, and per-tool-call exports would leave the same gap at a finer grain. Reloading before each delegation addresses freshness, which is not the failure, since the stem states the manifest already records the finished items. Treating every entry as untrustworthy discards the record entirely and returns the system to restarting from scratch, which is the behaviour the manifest exists to avoid."},
...
{d:"Agentic Architecture & Orchestration",q:"A healthcare startup is building an agent that can schedule appointments and access patient records. At what point should the agent require human approval?",o:["Only when the model reports a confidence score below 50% for the action it proposes","Before any action that modifies patient data or schedules real appointments","Only when the patient explicitly asks to be handed over to a human","Never: full autonomy is the design goal, and an approval step on each action defeats it"],a:1,
...
{d:"Agentic Architecture & Orchestration",q:"A coordinator delegates payment capture to a settlement subagent whose prompt instructs it to obtain human confirmation before capturing. Captures keep completing and no confirmation is ever put to anyone. Why is that?",o:["The subagent was spawned without the coordinator's history, so carrying the confirmation policy into its prompt is the change that produces the pause","Subagent communication routes through the coordinator, so a confirmation raised inside the subagent reaches nobody; the gate belongs before delegation","A hook inside the subagent should block the capture, since a blocked call is the pause that the confirmation instruction was asking for","The coordinator should review each capture once the subagent reports it, since an approval recorded against the completed call still bounds the exposure"],a:1,
e:"Task Statement 1.2 puts the coordinator at the hub, managing inter-subagent communication and routing all of it, which means a subagent has no channel of its own to a person. An instruction to confirm therefore has no addressee inside the subagent, and the confirmation cannot be produced no matter how the instruction is worded. The placement follows from the topology: the approval has to be taken at the coordinator before the capture is delegated, or the subagent has to return a proposed capture for the coordinator to put forward. Carrying the policy into the subagent's prompt is the Task Statement 1.3 context-passing mechanism aimed at the wrong problem, and the stem already grants that the subagent holds the instruction. A hook can block the call, which is deterministic and still obtains nobody's answer, so it converts a missing approval into a payment that never completes. Reviewing after the capture reverses the order that Task Statement 1.5 sets out, where interception redirects an action to human escalation before it runs rather than after."},
...
{d:"Agentic Architecture & Orchestration",q:"A provisioning agent has attempted the same access-grant step three times and been blocked each time by the same condition. Its policy plainly covers this request, and the customer has not asked to speak to anyone. What should it do?",o:["Keep working the step, since the policy covers this request and a covered case is one the agent was built to resolve without involving a person","Escalate once the customer's messages turn frustrated, since sentiment is the signal that shows the automated path has stopped working for them","Tell the customer the request cannot be completed and close the contact, since three identical failures show this path leads nowhere","Escalate now, because inability to make meaningful progress is an escalation trigger in its own right, whether or not policy covers the case"],a:3,
e:"Task Statement 5.2 lists three escalation triggers: a customer asking for a human, a policy exception or gap, and inability to make meaningful progress. The third stands alone, so an agent that is stuck escalates even where the policy is clear and the customer is content, and a covered case is not evidence that the agent can finish it. Continuing to work the step mistakes coverage for capability and repeats an attempt whose blocking condition has not changed across three tries. Waiting for frustration substitutes sentiment for case state, which Task Statement 5.2 names as an unreliable proxy. Closing the contact converts a stall into a refusal and asserts more than the agent has established, since what it knows is that this path is blocked, not that the request cannot be met by anyone."},
...
{d:"Claude Code Configuration",q:"A team runs Claude Code in VS Code through the extension rather than the CLI in the integrated terminal. Which capability does the extension give them that the terminal CLI does not?",o:["The full set of slash commands and skills, where the terminal CLI carries only the subset of them that runs without an interactive session","Claude's plan opens as a Markdown document in the editor where they can add inline comments before it begins making changes","A bash shortcut and tab completion for commands and paths, neither of which the terminal CLI makes available to a developer","A conversation history of its own, so a session opened in the panel stays isolated from anything started in the terminal"],a:1,
e:"The extension runs Claude Code as a graphical panel inside the editor, and plan review is the capability that panel adds: the plan arrives as a full Markdown document the developer can annotate with inline comments before any change is made, rather than scrolling past as terminal output. The other three describe the comparison backwards. Slash commands and skills are the CLI's strength, not the panel's, since the panel exposes only a subset of them. The bash shortcut and tab completion are both CLI features the panel does not have. And the two share one conversation history rather than keeping separate ones, which is why a conversation begun in the panel can be picked up in the terminal with resume."},
...
{d:"Claude Code Configuration",q:"A Claude Code settings.json lists several specific Bash commands in the permissions allow array. In the default permission mode, Claude proposes a Bash command that none of those entries covers. What happens?",o:["Claude Code refuses to run the command, because adding any allow rule closes the session to the entries named on that list and blocks everything else","Claude Code stops and asks the developer to approve it, because an allow rule grants automatic approval and does not deny what it omits","Claude Code runs it without a prompt, because an allow entry for one Bash command grants automatic approval across that whole tool","Claude Code has the model judge whether it is close enough to an allowed command, and runs it whenever the model decides that it is"],a:1,
e:"An allow rule is an automatic-approval rule rather than a boundary. It lets the command it names run without a prompt, and a command that no entry covers falls through to the ordinary permission prompt, where the developer approves or declines it. The mode that denies whatever has not been pre-approved is dontAsk, and the rule that stops a command outright is a deny rule, evaluated ahead of ask and ahead of allow so that no later allow entry can reopen it. An allow entry also covers the pattern it names rather than the whole tool, so permitting one Bash command does not quietly permit the rest. Permission rules are enforced by Claude Code itself rather than by the model, so nothing is admitted on a judgement that it resembles something already allowed."},
...
{d:"Context Management & Reliability",q:"Your team is concerned about a model update changing behavior in production. What deployment strategy minimizes risk?",o:["Update every production system to the new version at once, and roll back to the previous version if error rates or complaints rise afterwards","Use model version pinning in production and implement canary deployment: test the new version with a small percentage of traffic before full rollout","Never update the model version in production, so that behaviour stays fixed for the life of the deployment and no regression is possible","Let Anthropic decide when to update by leaving the model alias unpinned, since each release is checked for regressions before it reaches the alias"],a:1,
...
{d:"Claude Code Configuration",q:"A developer wants their own version of the team's /audit skill, carrying two extra checks that only they care about. The team's skill is checked into the repository and other contributors rely on it as it stands. What should the developer do?",o:["Add a variant under ~/.claude/skills/ under a name of its own, where it reaches that developer alone and the team's copy in the repository is untouched","Edit the team's SKILL.md in .claude/skills/ and leave that file permanently unstaged, so the two extra checks never reach a commit that other contributors pull","Set context: fork on the team's SKILL.md, so the two extra checks run in an isolated sub-agent and their output stays clear of other contributors' sessions","Copy the team's SKILL.md into the developer's own ~/.claude/CLAUDE.md, so the two extra checks load in each of their sessions without the repository changing"],a:0,
e:"A personal variant belongs in the user-scoped skills directory under a name of its own: it applies to that developer, it is never committed, and the team's skill keeps working unchanged for everyone else. Leaving a tracked file permanently unstaged does not create a variant at all, it edits the shared skill and leaves the working tree permanently dirty, so any checkout or broad add undoes it. context: fork is a real frontmatter option aimed at a different problem, keeping a verbose skill's output out of the main conversation, and it governs where output goes rather than which people a skill reaches. CLAUDE.md holds instructions and context rather than skill definitions, so pasting a SKILL.md into it does not give the developer a skill they can invoke by name."},
...
{d:"Prompt Engineering & Structured Output",q:"You are designing the review architecture for a service where a typical change touches several modules and also the data that moves between them. How should the review passes be arranged?",o:["Order the modules by how much of the change each one carries and review the largest first, so the closest reading lands where most of the edit sits","Run a local analysis pass over each module on its own, then a separate integration pass whose subject is the data crossing module boundaries","Run one pass per module and take the union of their findings as the finished review, since every line of the change is then read by exactly one pass","Have the session that wrote the change carry out the review, since it already holds the reasoning behind each module and needs no integration pass"],a:1,
e:"Splitting the work into per-module local passes plus a separate pass for what crosses module boundaries is the architecture the guide describes, and it is the arrangement that gives each module a full reading while still leaving something responsible for the data flow between them. Ordering by size is a real triage habit that changes which module is read first and nothing about whether the boundaries are read at all. Taking the union of per-module passes covers every line and still misses every interaction, because an inconsistency between two modules belongs to neither pass on its own. Handing the review to the session that produced the change is the least effective option available: a session retains its own reasoning from writing the code and is correspondingly unlikely to question decisions it has already committed to."},
...
{d:"Tool Design & MCP Integration",q:"An agent has to change one timeout constant inside a 900-line configuration module. The line declaring that constant appears nowhere else in the file. Which built-in tool applies the change, and on what grounds?",o:["Read the module in full and Write it back with the new value, because rewriting the whole file is what guarantees the change reaches the declaration reliably and in one pass","Write the module with the new value, because the built-in tool that produces file content is the one that puts a changed declaration onto disk in that module","Grep the module for the constant's declaration, because narrowing a 900-line module to the one line that defines the constant is the step that carries the change","Edit the declaring line, because a targeted modification anchored on text that occurs once is what the tool matches on and the rest of the module is left alone"],a:3,
e:"Edit performs targeted modifications and needs its anchor text to occur exactly once, which is the situation described, so it changes the declaration and leaves the other 899 lines untouched. Read paired with Write is the built-in combination for full file operations, and the guide reserves it for the case where Edit cannot find unique anchor text; the anchor here is unique, so rewriting the module spends the whole file on a one-line change. Write on its own is worse than that, because it replaces the module with whatever content is supplied and the 899 lines that were never read are lost. Grep searches file contents and does locate the declaration, but locating is not modifying and the change still has to be applied by a tool that writes."},
...
{d:"Claude Code Configuration",q:"A developer adds a /changelog entry point and it works for them every time. Another contributor on the same project does not see it at all. What best explains that?",o:["It was written into the project's CLAUDE.md, which supplies Claude Code with context and instructions rather than with definitions of this kind","The contributor's user-level CLAUDE.md takes precedence over the project's configuration and is holding the project-scoped entry out of their menu","It was created under ~/.claude/commands/, which is scoped to its author and is never carried to anyone else by the repository itself","It depends on a server declared in the project's .mcp.json which the contributor has not approved, so it stays unavailable on their machine"],a:2,
e:"User-scoped entries live in the author's home directory and are never version-controlled, which is exactly the pattern described: the author holds the file locally so it works for them, and nothing carries it across to anybody else. The other three explanations all fail the part of the scenario that says it works for the author. Something written into CLAUDE.md would not be invocable for anyone, since that file carries context rather than definitions. Configuration precedence is real but it settles which instructions win, not what appears in a menu, and it would not spare the author either. MCP servers are configured in .mcp.json and supply tools, so one left unapproved is not what makes /changelog invisible."},
...
{d:"Claude Code Configuration",q:"You want machine-parseable structured output from Claude Code in CI for automated posting as inline PR comments. Which flags should you use?",o:["--input-format stream-json with --max-turns","--output-format json with --json-schema","--permission-mode plan with --max-budget-usd","--system-prompt-file with --setting-sources"],a:1,
...
{d:"Prompt Engineering & Structured Output",q:"You want to guarantee that Claude calls a tool rather than returning conversational text. Which tool_choice setting should you use?",o:["tool_choice: 'auto'","tool_choice: 'any'","tool_choice: {type: 'tool', name: 'specific_tool'}","tool_choice: 'none'"],a:1,
...
{d:"Prompt Engineering & Structured Output",q:"Your extraction schema has a 'category' field with enum values ['invoice', 'receipt', 'contract']. Some documents are certainly one of those three, but their text never establishes which, and the model picks the nearest value instead of reporting that it could not tell. How should you fix the schema?",o:["Drop the enum constraint and take the category as free text, so the model is never made to choose among three values the document does not settle between","Add an 'other' member paired with a free-text detail field, so a document belonging to none of the three categories is recorded instead of being mis-filed","Add an 'unclear' member to the enum, so a document whose category its text leaves open is returned as undetermined rather than as the nearest value","Add a validation step that rejects any category the document's text does not corroborate and retries the extraction with that error appended to the prompt"],a:2,
...
{d:"Prompt Engineering & Structured Output",q:"A review prompt sets out in prose the four fields every finding must carry, and the model still returns findings that drop a field and change shape between runs. Which two changes address that most directly?",o:["Restate the four field names a second time immediately before the diff, so the requirement is the last thing the model reads before it generates","Add two or three worked examples that render a complete finding in the exact four-field shape the output is meant to take","Describe each of the four fields at greater length, so the model is given a fuller specification of what each one should contain","Define the finding as a JSON schema on an extraction tool and read the findings out of the tool_use response rather than out of prose","Move the four field names into the system prompt, leaving the diff and the instruction to review it in the user turn"],a:[1,3],type:"mr",
e:"Two independent mechanisms are at work and the prompt is using neither. Worked examples demonstrate the shape rather than describing it, which is what the guide names as the most effective route to consistently formatted output, and a model that has seen a finding rendered correctly reproduces its shape far more reliably than one that has only read about it. A JSON schema attached to a tool removes the question from prose altogether: the guide names tool use with a schema as the most reliable approach for structured output, and a field the schema requires cannot simply be dropped. The three wrong answers all leave the output governed by prose. Repeating the field names moves an instruction without changing its kind. Describing the fields at greater length is more of exactly what has already failed. Moving the names into the system prompt changes where the specification sits rather than how the model is shown to satisfy it."},

{d:"Prompt Engineering & Structured Output",q:"Your application instructs Claude in the system prompt to return only a JSON object. The model produces valid JSON but adds commentary after it. A migration to tool use is scheduled for a later release, so the fix has to work with the current prompt-based call. What additional technique should you use?",o:["Set max_tokens to limit the response length","Add stop_sequences to stop generation after the JSON closes (e.g., a trailing newline)","Add 'return only JSON' to the prompt","Switch the call to tool use with a JSON schema, so the object arrives without trailing commentary"],a:1,
...
{d:"Tool Design & MCP Integration",q:"You want to expose your team's Jira project data to Claude Code through MCP. Your team already uses a standard Jira workflow. Should you build a custom MCP server or use an existing community server?",o:["Always build custom for better control and security","Use an existing community MCP server for standard integrations like Jira; reserve custom servers for team-specific workflows","Expose Jira as an MCP prompt run from the slash menu, since prompts are discovered from connected servers and need no server of their own","Configure Jira access through CLAUDE.md instructions"],a:1,
...
{d:"Agentic Architecture & Orchestration",q:"An agent designed to draft emails is also given tools to send, delete, and schedule emails 'for convenience.' The agent accidentally sends a draft mid-conversation. What principle was violated?",o:["Principle of least surprise: the agent behaved unexpectedly","Scoped tool access: the agent held tools beyond those its drafting role required","Separation of concerns: the prompt mixed email drafting and sending logic","Error containment: the agent should have validated before sending"],a:1,
e:"Task Statement 2.3 of the exam guide names this scoped tool access: giving agents only the tools needed for their role, with limited cross-role tools for specific high-frequency needs. A drafting agent should hold read and write access to drafts only, not send, delete, or schedule. Providing unnecessary high-consequence capabilities increases the blast radius when the model makes an error."},
...
{d:"Agentic Architecture & Orchestration",q:"A developer builds an agent that autonomously sends outbound marketing emails without human review. This violates which key agentic safety principle?",o:["Scoped tool access: the agent holds more tools than its role needs","Human-in-the-loop: consequential, irreversible outbound communications require human confirmation","Context management: the email content consumes too many tokens","Error containment: email failures should be caught and logged"],a:1,
...
{d:"Agentic Architecture & Orchestration",q:"A retrieval subagent is scoped to a single search tool so it cannot act outside its role. Whenever that search returns nothing usable the subagent escalates, and the coordinator finds it has no second avenue to offer either. What does this show about the design?",o:["A subagent recovers only from failures its own tool scope can address, so the scope decides which errors it must propagate","Scoping a subagent to a single tool removes its ability to retry that tool at all, so even a transient failure has to be handled one layer up","Recovery belongs with whichever agent holds the conversation history, so a subagent is expected to escalate every failure it meets","The escalation is a decomposition fault, since a subtask that can fail this way should never have been delegated as a single unit"],a:0,
e:"Restricting a subagent to the tools its role needs is sound practice, and recovering locally before propagating is also sound practice, but the first bounds the second: a subagent whose scope holds one search tool has nothing to fall back on when that search comes back empty, so the failure it cannot resolve is the one its scope never gave it the means to resolve. Naming that boundary is what tells a designer whether to widen the scope or to give the coordinator an alternative. Scoping does not remove the ability to retry the tool the subagent does hold, so a transient failure remains recoverable locally. Recovery is not sited by which agent holds the conversation history; a subagent is expected to resolve what it can and propagate only what it cannot. Decomposition is a real concern, but a single retrieval is the right size for one delegation and the gap is in what the retriever was equipped with."},
...
{d:"Claude Code Configuration",q:"A skill that summarises the current review thread is set to context: fork so its verbose output stays out of the main session. Its summaries now omit decisions the team reached earlier in that conversation. What explains this?",o:["The forked run inherits the conversation but drops any turn that has already been compacted, so only the older decisions go missing","The fork returns only a summary of its work to the main session, so the detail is lost on the way back rather than on the way in","The fork isolates context in both directions, so the skill runs without the conversation history unless its prompt carries it","The skill is loaded automatically rather than typed, so it begins before the conversation history has finished loading into it"],a:2,
e:"Setting context: fork runs the skill in an isolated sub-agent, which is what keeps its verbose output out of the main session. That isolation is not one-directional: the forked run is a sub-agent context, and a sub-agent does not inherit the conversation history of the session that invoked it, so anything the skill needs about the thread has to arrive in its own prompt. Compaction is a real mechanism, but it drops the oldest turns from a session rather than deciding what a fork inherits, and the skill here is missing recent decisions as well. The fork does return a summary to the main session, but that governs what comes back rather than what went in. Automatic loading is governed by disable-model-invocation and changes who starts the skill, not what context it starts with."},
{d:"Claude Code Configuration",q:"A monorepo's standards have grown large enough that loading all of them at the start of every session measurably reduces the room left for the work itself. Which property of a .claude/rules/ file with a paths field addresses that?",o:["Running /compact once the standards are no longer being consulted reclaims the room they occupy, so a long session recovers the space partway through","Referencing each standards file from its own package's CLAUDE.md with @import, so that package's file carries only the standards its maintainers need","Splitting the standards into separate skills, so that each set is loaded at the point a developer invokes it by name and at no other time","Its rules are read in only while a file its glob matches is being edited, so a session that never opens such a file never carries them at all"],a:3,
e:"A paths field makes a rule conditional on what is being edited, so the cost of a standards file is paid only in the sessions that touch the files it governs and a session working elsewhere carries none of it. The three wrong answers are all real mechanisms doing something else. /compact reduces context that has already accumulated, which recovers room after the cost has been paid rather than avoiding it. @import genuinely keeps a CLAUDE.md modular and is the right answer to a different task statement, but it divides the standards by package rather than making any of them conditional, and a package's own file still loads in full. Skills are invoked on demand, which is the wrong shape for standards that have to apply whether or not somebody remembers to ask for them."},
...
{d:"Prompt Engineering & Structured Output",q:"An extraction pipeline forces its extract_fields tool with tool_choice on every document and marks every schema field required. Reviewers keep finding confident values in fields the source documents never mention. What does this indicate?",o:["Forcing a named tool suspends validation of that call, so fields with no support in the document pass through instead of being rejected","The tool description is too thin, so the model is guessing what belongs in each field rather than reading the value out of the document","Validation should run after extraction and reject any document whose fields cannot be traced to its text, since prevention is not available here","Forcing the call removes the option to decline, so required fields must be filled and the schema needs nullable or unclear values"],a:3,
e:"Forcing a named tool guarantees the extraction runs, and marking a field required guarantees the model emits something for it. Together those two guarantees leave the model no way to record that a document is silent on a field, which is the pressure that produces an invented value. The schema has to supply the affordance the forced call takes away, by making genuinely absent fields nullable and by giving an enumeration an unclear member. Forcing a tool does not suspend validation of its input schema. A thin tool description is a real cause of poor tool selection and poor argument quality, but here the model is filling fields the document does not cover at all, which is a schema-design pressure rather than a description gap. Validation after the fact routes bad output for review, and prevention is available here, which is what makes the last option wrong on its own terms."},
...
{d:"Context Management & Reliability",q:"A support session handles three separate order issues at once. The agent keeps a case-facts block, yet it applies the replacement approved for one order to a second order that was only ever queried. What should change?",o:["Extend the case-facts block with a further paragraph covering all three of the issues together, so no single fact is left outside the retained context","Persist each issue as its own structured entry keyed by order identifier, so a fact from one issue cannot be read as a fact about another","Handle the three issues in three separate sessions, since one conversation cannot reliably carry more than a single open case at a time","Have the agent re-read the order history before each action, so the most recent lookup always settles which order is being worked on"],a:1,
e:"A flat case-facts block preserves facts but not which issue each fact belongs to, so an approval attached to one order can be read as applying to another. Extracting and persisting structured issue data, the order identifiers, amounts and statuses, into a separate context layer is what keeps a multi-issue session distinguishable. Describing all three issues in one paragraph retains the same ambiguity at greater length. Splitting into three sessions discards the shared customer context and treats a limitation of the record-keeping as a limitation of the conversation. Re-reading order history reports the state of an order but not which issue the customer authorised, which is the fact that went missing."},
