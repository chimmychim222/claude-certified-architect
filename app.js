...
e:"PostToolUse hooks that normalize heterogeneous data formats (Unix timestamps, ISO 8601, numeric status codes) from different MCP tools ensure the agent always sees consistent data. This prevents the model from having to reason about different formats and improves the quality of its subsequent decisions. Current Claude Code documentation describes PostToolUse as running after a tool call returns and able to add context rather than rewrite the result, with PreToolUse the hook that can modify a tool call before it executes."},
...
<ul>
<li><strong>PostToolUse hooks:</strong> Intercept tool results for data transformation before the model processes them. Example: normalizing heterogeneous data formats (Unix timestamps, ISO 8601 dates, numeric status codes) from different MCP tools into a consistent format. Current Claude Code documentation describes PostToolUse as running after a tool call returns and able to add context rather than rewrite the result, with PreToolUse the hook that can modify a tool call before it executes.</li>
