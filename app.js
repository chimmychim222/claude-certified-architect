...
{d:"Claude Code Configuration",q:"You're about to implement a library migration affecting dozens of files. Before coding, you want to explore the codebase safely. What's the recommended approach?",o:["Write a test suite covering the migration's expected behaviour first, then iterate by feeding the failures back until the suite passes","Use plan mode for investigation and design, then switch to direct execution for implementation: this prevents costly rework","Stay in plan mode for the whole migration, since leaving it discards the plan Claude Code built while investigating","Write the migration's conventions into CLAUDE.md first so that every later edit picks them up"],a:1,
