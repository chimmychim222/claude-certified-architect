...
e:"Option 1 is Task 1.5's knowledge point on deterministic vs. probabilistic compliance; option 2 is Task 1.5's separate knowledge point on hook patterns that intercept outgoing tool calls before they execute. These are independent facts — a candidate could know one without the other. Option 3 misapplies a real Task 1.1 concept (iteration limits) to a mechanism hooks have no effect on. Option 4 misdescribes a real file's role: CLAUDE.md carries natural-language instructions and context, not hook code. Option 5 conflates hooks with a real Task 2.2 mechanism (the isRetryable flag on structured MCP error responses) — retry logic lives in the agent loop, not in the hook."},
...
e:"Path-specific rules use .claude/rules/ with YAML frontmatter glob patterns (Task 3.3 Knowledge) — not a note inside CLAUDE.md, which always loads in full regardless of what's being edited. The conditional-loading benefit — loads only when editing matching files, reducing irrelevant context and token usage (Task 3.3 Knowledge) — is the second half of setting it up correctly. Option 4 is a real mechanism (directory-level CLAUDE.md, Task 3.1) at the wrong scope — notebooks scattered across directories need a file-type rule, not a per-directory one. Option 5 is a real mechanism (Task 2.4) at the wrong problem — MCP resources expose content to agents, they don't scope Claude Code's own instruction loading."},
...
e:"Project-scoped .mcp.json is version-controlled and therefore the right home for shared team tooling; ~/.claude.json is user-scoped and the right home for personal or experimental servers. The token belongs in an environment variable referenced by expansion — repository access control is not a substitute for keeping a secret out of the file. Tools from all configured MCP servers are discovered at connection time and are available simultaneously, so no replacement is needed. CLAUDE.md carries instructions and context, not server configuration."},
...
e:"Directory-level CLAUDE.md is scoped to its own subdirectory (Task 3.1 Knowledge). Glob-pattern .claude/rules/ files win when the convention is about file type rather than location (Task 3.3 Knowledge: the advantage of glob-pattern rules over directory-level CLAUDE.md files for conventions that span multiple directories). Option 3 invents an exclusivity that doesn't exist — the two mechanisms coexist. Option 4 misstates path-scoped loading, which activates only when editing matching files (Task 3.3 Knowledge), not once per session. Option 5 reverses directory-level scope — it's confined to its own directory, not broadcast project-wide."},
...
e:"A, B, and C are each Task 2.3 knowledge/skill bullets: too many tools degrades selection reliability (A), agents misuse tools outside their specialization (B), and restricting each agent's tool set to its role prevents that misuse (C). D is plausible-sounding but wrong: the guide attributes this degradation to tool count and decision complexity, not model capability. E is real Task 2.3 content at the wrong scope: 'any' guarantees some call is made, it doesn't reduce how many tools compete for selection. F is real Task 1.5 content, wrong problem: logging is retrospective, it doesn't improve upfront selection."},
...
e:"A, B, and C are each drawn from Task 2.1's account of what a tool description should include: input formats, example queries, and boundary explanations that distinguish the tool from alternatives. D is wrong: Task 2.1 itself warns that keyword-sensitive system prompt wording can create unintended tool associations — steering selection from the system prompt is the risk to review for, not the fix. E is a real practice (audit logging) aimed at the wrong problem: it records what already happened, it gives the model nothing to reason with beforehand. F is real schema-validation content (Task 2.4) at the wrong problem: it constrains what a valid call looks like, it doesn't tell the model when to pick this tool over another."},
...
e:"A is correct per Task Statement 5.1's knowledge that progressive summarization risks condensing numerical values, percentages, dates, and customer-stated expectations into vague summaries. B is correct per Task Statement 5.1's skill of extracting transactional facts into a persistent case-facts block that is never summarized away. C is a real lever, summarization frequency, applied to the wrong problem: summarizing more often frees window space but makes precision loss worse, not better. D falsely describes the context window's actual behavior; it has no such mechanism for protecting specific facts from summarization. E is a real technique that belongs to Task Statement 4.2's few-shot prompting, and it addresses output phrasing rather than the underlying data loss."},
...
e:"A is correct per Task Statement 5.1's knowledge that the lost-in-the-middle effect causes models to give less attention to content buried in the middle of long inputs. B is correct per Task Statement 5.1's knowledge that tool results accumulate in context and consume tokens disproportionately to their relevance, mirroring the guide's own 40-field example. C misapplies a real technique to the wrong problem: progressive summarization frees window space, it does not fix reduced attention to mid-context content. D falsely describes the Messages API's actual behavior, which performs no automatic compression of tool_result blocks. E is true but belongs to Task Statement 1.2's coordinator hub-and-spoke pattern, not to preserving critical information across a long interaction."},
...
e:"A is correct per Task Statement 5.1's skill of trimming verbose tool outputs to only relevant fields before they accumulate in context. B is correct per Task Statement 5.1's skill of modifying upstream agents to return structured data instead of verbose content and reasoning chains when downstream agents have limited context budgets. C is a real mechanism, prompt caching, aimed at the wrong problem: caching a static prompt or tool definition avoids reprocessing that fixed text on repeated calls, it has no effect on how many fields a tool's response contains. D falsely describes the Messages API's actual behavior; it performs no automatic pruning of tool results. E is true but belongs to Task Statement 2.3's scoped tool access, and it does not address either symptom described."},
...
e:"A and B describe the two halves of this configuration and are independent Task 2.3 facts: forced selection guarantees extract_metadata specifically on the first turn (A), and switching to 'auto' afterward restores free choice for enrichment steps (B). C is real Task 4.3 content (schema validation) at the wrong problem: tool_choice controls which tool runs, not whether its output is schema-valid. D contradicts the premise: forced tool_choice is exactly the guide-cited mechanism that does guarantee this on its own; hooks address a different need, such as blocking policy-violating calls. E overstates the requirement: per the guide's own skill bullet, forcing is scoped to the first turn only, not every subsequent turn."},
...
e:"A, B, and C are each independently true about Task 2.3's tool_choice modes: 'auto' allows text instead of a tool call (A), 'any' guarantees some tool but not a specific one (B), and forced selection guarantees the named tool specifically, typically relaxed back to 'auto' afterward (C). D directly contradicts A — auto explicitly allows text-only responses, this is a common misreading, not a real alternative behavior. E is real (schema validation exists, Task 2.4/4.3) applied to the wrong mechanism: tool_choice controls selection, not input validation. F is real (Task 1.5 hooks intercept tool calls) at the wrong scope: hooks act on calls as they happen, they don't retroactively rewrite which mode governed a turn already past."},
...
e:"A and B are independent elements from Task 2.1's account of description quality: input format plus example query (A), and a boundary explanation (B). C is the Task 1.1 pre-configured decision-tree anti-pattern applied to tool selection. D misapplies Task 2.3's tool-count principle at the wrong scope and overstates it as absolute. E misuses a real API parameter (output length) for a selection problem it can't affect."},
...
e:"This is the guide's own stated advantage — glob rules apply to files by type regardless of directory location, while directory-level CLAUDE.md only affects files in that specific directory (Task 3.3 Knowledge and Skill); the token-usage benefit follows from the same conditional loading (Task 3.3 Knowledge). Option 3 misdescribes the matching mechanism: paths in .claude/rules/ frontmatter are matched relative to the project root, not the absolute filesystem path, so the same pattern behaves identically regardless of where the repository is cloned. Option 4 misdescribes directory-level CLAUDE.md scope as cascading, when it is confined to its own directory. Option 5 invents a priority rule between the two mechanisms that the guide never states — the real advantage is about scope, not precedence."},
...
e:"Option 1 is what tool-use schema enforcement actually guarantees: syntactic validity, with no malformed structure or type mismatches. Option 2 is the guide's explicit corollary — strict schemas do not prevent semantic errors like wrong-field placement or totals that don't sum, which is why extraction pipelines need additional validation logic beyond the schema itself. Option 3 is a false capability claim: no schema mechanism checks an extracted value against the source document's actual content. Option 4 misapplies a real Task 4.3 mechanism (nullable optional fields) to the wrong problem — nullability stops fabrication when data is absent, not misplacement of data that is present. Option 5 misapplies another real Task 4.3 mechanism (tool_choice) to the wrong problem — tool_choice controls whether and which tool gets called, not where an extracted value lands once the tool is called."},
...
e:"Option 1 is correct: it directly states the guide's named cause — retained generation-reasoning context biases the same session against its own decisions. Option 2 is correct: it directly states the guide's named fix, and explicitly rules out extended thinking as a substitute — an independent instance is what actually helps. Option 3 is a false behavior claim the guide directly rules out — extended thinking within the same session does not confer independence from that session's own reasoning. Option 4 misapplies a real API parameter (temperature) — it changes output randomness, not whether the session is biased toward its own prior decisions. Option 5 names a real Task 4.6 mechanism (multi-pass review) but at the wrong scope — that architecture fixes attention dilution across many files, a different failure mode from same-session reasoning bias."},
...
e:"Option 1 is correct: format or structural mismatches are exactly the case the guide names as where retry-with-feedback succeeds — the model can act on the specific error shown to it. Option 2 is correct: the guide explicitly contrasts this with information that's absent from the source — retry cannot manufacture data that was never there. Option 3 is a false behavior claim — nothing about the mechanism improves with attempt count; if the data isn't in the document, the eleventh attempt fails exactly like the third. Option 4 names a real Task 4.3 fix (nullable optional fields) but at the wrong scope — that's a schema design decision, not a description of what the retry loop itself does or doesn't accomplish. Option 5 misapplies a real API parameter (max_tokens, Task 4.3) — it controls output length, not whether information exists to extract."},
...
e:"A and B are two of the independent, co-equal components Task Statement 5.3 names for structured error context: failure type and partial results. C misapplies a real mechanism, retry with backoff, to a non-transient failure; validation errors do not resolve on retry. D falsely describes tool_choice's actual behavior; it has no such status-code routing function. E is true, scoped tool access is a real, documented practice, but it belongs to Task Statement 2.3, not to what a failure report should contain."},
...
e:"Task 1.2 names the coordinator's role as task decomposition, delegation, and result aggregation, among other duties — options 1 and 2 draw on two of those, and they are independent: a candidate could place decomposition correctly with the coordinator while still misplacing aggregation. Options 3 and 5 are the right idea at the wrong owner — producing findings and executing tool calls are what a subagent does within its own isolated context (Task 1.3), not the coordinator. Option 4 is real Task 1.5 content (PostToolUse normalization) at the wrong scope — that is a per-tool-result transformation step, not a role the coordinator itself performs."},
...
e:"Both fixes come from Task 1.4's skills: a programmatic prerequisite gate guarantees the verification step runs before the financial action, and a structured handoff summary is what Task 1.4 specifies for mid-process escalation so a human reviewer without transcript access can act immediately. Option 3 is the anti-pattern the fix replaces — prompt instructions provide probabilistic, not deterministic, compliance. Option 4 misreads the problem as one of information volume rather than structure. Option 5 is real Task 1.5 content (PostToolUse normalization) at the wrong problem — normalizing timestamp formats doesn't fix a handoff that's missing root-cause and recommended-action fields entirely."},
...
e:"Task 1.5's skill bullet names heterogeneous timestamps, status codes, and other inconsistently-formatted fields as PostToolUse normalization targets, and the three are independent — a hook author could normalize timestamps and forget status codes, or vice versa. Option 4 misreads format variance as a transient failure; retrying does nothing, since the same tool may format identically next time too. Option 5 is real Task 2.2 content (errorCategory/isRetryable for structured MCP error responses) at the wrong problem — these are valid successful results, not errors. Option 6 confuses normalization with the separate Task 1.5 mechanism of pre-execution tool-call interception — this data isn't policy-violating, it's just differently formatted."},
...
e:"This applies the principle of least privilege: give the classification agent only what it needs for the 90% common case (simple static-blocklist lookups) while preserving the coordinator pattern for the 10% cases requiring live threat-feed queries. Batching (B) creates blocking dependencies — the classification agent must wait for a full batch before any single check completes. Full threat-feed access (C) over-provisions the agent with capability it rarely needs. Proactive caching (D) can't reliably predict which URLs will need a live query, and threat feeds change too quickly to cache reliably in advance."},
...
e:"Task 1.4's skill bullet names all three correct facets — decompose into distinct items, investigate in parallel using shared context, synthesize into one resolution — and each is independently checkable. Option 4 is real Task 1.6 content (dynamic adaptive decomposition for open-ended investigation) at the wrong problem — this request's two concerns are already known upfront, not discovered mid-investigation. Option 5 misapplies escalation: Task 5.2's actual triggers are explicit customer requests, policy gaps, or inability to progress — not multiple concerns in one message. Option 6 is a real mechanism at the wrong scope — splitting into separate coordinators with independent context windows is the opposite of the shared context the correct answer calls for."},
...
e:"Task 1.4 specifies both the required content and the reason for it — these are independent claims, since a candidate could know the required fields without realizing raw transcripts are an unreliable substitute, or vice versa. Option 3 is real Task 1.5 content (PostToolUse normalization) at the wrong problem — normalizing tool-result formats has nothing to do with what a human-facing summary should contain. Option 4 is real Task 1.2 content (routing subagent communication through the coordinator for observability) at the wrong scope — that governs inter-agent traffic, not the contents of a human handoff document. Option 5 misapplies Task 1.1's iteration-cap safety-net concept to a timing question it has no bearing on."},
...
e:"A and B are independent Task 2.1 fixes: A is the skill of writing descriptions that clearly differentiate each tool's purpose, expected inputs, outputs, and when to use it versus alternatives; B is the separate skill of renaming a tool so its name alone signals a distinct purpose — achievable without touching the description text at all, which is what keeps it independent of A. C is the Task 1.1 pre-configured decision-tree anti-pattern applied to tool selection, not a Task 2.1 fix. D is a real Task 1.5 mechanism (PostToolUse hooks) at the wrong problem: it normalizes results after a call, it doesn't influence which tool gets picked. E is real Task 2.3 content at the wrong scope: 'any' guarantees some tool is called, not the correct one between two ambiguous options."},
...
e:"A and B are independent Task 2.3 skills: restricting the primary tool set to role-relevant tools (A), and separately allowing limited, scoped cross-role tools for high-frequency needs while routing complex cases through the coordinator (B) — B is a deliberate, bounded exception to a strict reading of A. C overcorrects: it contradicts that exception, even though 'route through the coordinator' alone is correct in the general case. D is a real Task 4.2 technique (few-shot examples) aimed at the wrong problem — coaching around 18-tool complexity instead of reducing the decision space. E is a real Task 5 concept (context window) that doesn't reduce how many tools compete for selection."},
...
e:"User-scoped commands in ~/.claude/commands/ are personal and are never committed to version control or shared with teammates who clone the repo — exactly what a private, individually-styled command needs. Option B is the opposite choice: project-scoped commands in .claude/commands/ are version-controlled and automatically shared with every developer who pulls the repo, which is what this engineer is trying to avoid. Option C confuses two different mechanisms — skills are frontmatter-configured, on-demand sub-agent workflows, not the way plain slash commands are defined. Option D (CLAUDE.md) holds project instructions and context, not command definitions."},
...
e:"Glob patterns match by type or name pattern, not folder or manual enumeration (Task 3.3 Knowledge). Conditional loading (Task 3.3 Knowledge) means unrelated files never pull in Storybook guidance. Option 3 misdescribes glob matching as manual listing. Option 4 invents a restriction — any glob pattern is usable, there's no official eligible-type list. Option 5 misdescribes evaluation as cached-at-open rather than evaluated against whichever file is actually being edited."},
...
e:"/memory directly surfaces which files are loaded — the fastest way to confirm whether the project-level CLAUDE.md is even in scope for this engineer (Task 3.1 Skill: using the /memory command to diagnose inconsistent behavior across sessions). If the rules were written into the engineer's personal ~/.claude/CLAUDE.md instead of the project-level file, they were never shared with the team in the first place (Task 3.1 Knowledge: user-level settings apply only to that user and are not shared via version control). /compact is a context-management tool (Task 5.1) and has nothing to do with which files load. Re-cloning is wrong — CLAUDE.md is read fresh at the start of every session, not just the first checkout. Skills are on-demand, task-specific workflows (Task 3.2) — the wrong tool for always-active architecture rules, which is exactly what CLAUDE.md is for."},

{d:"Claude Code Configuration",q:"A skill in .claude/skills/ surveys a codebase. It prints hundreds of lines into the session, occasionally writes files a reviewer did not expect, and produces confusing output when a developer invokes it with no arguments. Which three frontmatter settings address those three complaints?",o:["context: fork, so the survey runs in an isolated sub-agent and its output stays out of the main conversation","allowed-tools, restricted to read operations so the skill cannot write files while it runs","argument-hint, so a developer invoking the skill bare is prompted for the parameter it needs","A longer description, so Claude selects the skill less often and it runs only when clearly appropriate","Relocating the skill to ~/.claude/skills/, so its output affects only the developer who runs it","Appending /compact to the end of the skill body, so context is reclaimed once it finishes"],a:[0,1,2],type:'mr',
e:"Each correct option maps to one complaint: forked context isolates verbose output, allowed-tools bounds what the skill may do during execution, and argument-hint handles the bare invocation. The distractors are all real mechanisms aimed at the wrong problem — description governs selection, not verbosity; the user-scoped path changes who is affected, not how much is printed; /compact reclaims context after the damage rather than preventing it."},

{d:"Claude Code Configuration",q:"You've been asked to migrate a data-processing pipeline from a single nightly cron job to an event-driven streaming architecture. The change touches roughly 30 files, and there are two viable messaging systems you could adopt, each implying different infrastructure and error-handling patterns. Which approach should you take?",o:["Enter plan mode to explore the current pipeline, weigh the two messaging approaches, and design an implementation strategy before making changes","Pick whichever messaging system the last team that did a similar migration used, and start implementing directly without further exploration","Use direct execution with a single exhaustive upfront instruction listing every file to change and exactly how","Ask a senior engineer to describe the right architecture in a five-minute conversation, then implement directly from that description"],a:0,
e:"Plan mode is designed for exactly this shape of task: changes spanning roughly 30 files, and an architectural decision — which messaging system to adopt — with more than one defensible answer and consequences that are expensive to unwind once code is written around the wrong choice. Exploring the current pipeline first, in plan mode, lets you weigh both messaging approaches against how the rest of the pipeline actually works before committing. Option B substitutes precedent from an unrelated migration for actually understanding this pipeline's specific constraints. Option C assumes the right file-by-file structure is already known well enough to specify exhaustively upfront, which is precisely what this task hasn't yet determined. Option D treats a five-minute verbal description as a substitute for exploring the codebase directly, which risks missing constraints that only show up when you actually look at the code."},
...
e:"--output-format json paired with --json-schema is the documented way to guarantee machine-parseable structured output from Claude Code in a CI context — the schema constrains the response shape so a downstream script can consume it reliably. Option A solves a different problem (Claude Code hanging on interactive input) but says nothing about output format, so the parsing failure remains. Option C relies on the model reliably following a prompt instruction to self-format as JSON, which has no structural guarantee. Option D treats the symptom by scraping free-form text instead of fixing the actual mismatch between what Claude Code outputs and what the script expects."},
...
e:"Option 1 is correct: this matches the guide's statement that batch fits non-blocking, latency-tolerant workloads. Option 2 is correct: blocking workflows need guaranteed latency, which batch (up to 24-hour window, no SLA) cannot provide. Option 3 is correct: an independent fact — the batch API's no-multi-turn-tool-calling limitation applies regardless of which workflow is being discussed. Option 4 is a false claim — there is no guaranteed maximum turnaround; the guide states no guaranteed latency SLA, only an up-to-24-hour window. Option 5 misapplies a real Task 4.3 mechanism (tool_choice) — it controls whether/which tool gets called, not request latency or blocking behavior. Option 6 is a true statement, but it answers Task 4.6 (multi-pass review architecture), not Task 4.5 — it's about attention dilution across many files or items, not API selection."},
...
e:"Option 1 is correct: it directly addresses attention dilution — per-file passes guarantee uniform depth regardless of file position. Option 2 is correct: it directly addresses the contradictory-finding symptom — a dedicated cross-file pass is what catches the same pattern being judged two different ways. Option 3 names a real Task 4.6 mechanism (independent review instance) but at the wrong problem — that fixes same-session reasoning bias, not attention dilution across many files in one pass; this scenario doesn't establish the reviewer is the same session that wrote the code. Option 4 misapplies a real API parameter (max_tokens) — it bounds output length, it doesn't redistribute the model's attention across a long input. Option 5 names a real Task 4.4 mechanism (detected_pattern field) at the wrong task — that field supports longitudinal false-positive analysis across many review runs over time, not fixing attention dilution within a single pass."},
...
{d:"Prompt Engineering & Structured Output",q:"You ask Claude to review a 60-page technical specification document in a single request and flag every internal inconsistency. The output catches contradictions in the first ten pages but misses several in the later sections, and one inconsistency it does catch on page 45 is described differently than an identical one it missed on page 12. What is the most effective way to improve this?",o:["Increase max_tokens so the response can be longer and cover more of the document","Split the review into sequential per-section passes that build a running list of claims, followed by a dedicated cross-section pass that checks the accumulated list for contradictions","Ask the model to review the document twice in the same conversation and combine both answers","Shorten the document by removing sections unlikely to contain inconsistencies before review"],a:1,
e:"Long single-pass reviews over large documents suffer from attention dilution and inconsistent depth across sections, similar to the pattern seen in large multi-file code reviews. Splitting into focused sequential passes — first building a structured record of claims section by section, then running a dedicated cross-section pass against that record — catches contradictions a single pass misses and produces consistent treatment of identical patterns. A larger max_tokens budget affects output length, not how carefully earlier sections were read; re-reviewing in the same conversation retains the same reasoning context and biases rather than providing an independent check."},
...
{d:"Agentic Architecture & Orchestration",q:"Your agent drafts partnership contracts and can send them to external counterparties by email. The workflow requires internal legal sign-off before any contract leaves the building, but production logs show the agent has skipped this step and sent unapproved drafts twice in the past month despite a system-prompt instruction saying sign-off is required first. What change would most reliably prevent this?",o:["Rewrite the system-prompt instruction in stronger language, emphasising that sign-off is mandatory","Add a programmatic prerequisite that blocks the send_email tool from firing until a get_legal_signoff tool has returned an approved status for that specific contract","Add few-shot examples showing the agent always checking sign-off status before sending","Remove the agent's ability to attach documents to emails, forcing a human to attach them manually"],a:1,
e:"When a specific step must happen before another for a critical business reason — here, legal sign-off before external communication — programmatic enforcement (a hook or prerequisite gate that blocks the downstream tool call until the prerequisite condition is verified) provides a deterministic guarantee. Prompt-based approaches, even strongly worded ones with examples, are probabilistic: the model can still skip the step under some conditions, which is exactly what happened here. Removing the attachment capability entirely is a disproportionate workaround that breaks the agent's legitimate function."},
...
{d:"Prompt Engineering & Structured Output",q:"Your automated code review prompt includes the instruction 'only report high-confidence findings.' Developers say the tool still flags too many non-issues, and they've started ignoring its output. What is the most effective fix?",o:["Add 'be more conservative' to reinforce the existing instruction","Replace the subjective instruction with explicit, testable criteria that define exactly which categories of issues to report and which to skip","Lower the model's temperature to reduce variability","Increase max_tokens so the model can explain its reasoning more fully"],a:1,
e:"Vague qualifiers like 'high-confidence' or 'be conservative' are interpreted inconsistently by the model and don't reduce false positives. Explicit, testable criteria — specifying which categories to report (e.g., bugs, security issues) and which to skip (e.g., minor style, local patterns) — produce consistent, precise output. High false-positive rates in one category erode developer trust in all categories, which is why precise criteria matter more than reassurance words."},
...
e:"A and B are independent, both Task 2.1: A addresses the ambiguous/overlapping description cause, B addresses the separate keyword-sensitive system prompt cause — both are genuinely present in this scenario. C is a real Task 1.5 mechanism (PostToolUse hooks), wrong problem: it acts on results after a call, not on which tool gets picked. D misapplies Task 2.3's too-many-tools principle at the wrong scope — that principle addresses agents with excessive tool counts like 18+, not two ambiguous tools — and overstates it as absolute. E is real Task 2.3 content at the wrong scope: forced selection fixes a fixed first-step ordering problem, not ongoing ambiguity between two peer tools, and would permanently disable fetch_billing_document."},
{d:"Tool Design & MCP Integration",q:"An MCP tool call to a downstream service fails. The tool currently returns the generic message 'Operation failed' with no further detail. What change to the error response most improves the agent's ability to recover?",o:["Return the raw HTTP status code and nothing else","Return structured error metadata — an errorCategory of 'transient', an isRetryable: true flag, and a human-readable description — so the agent can decide whether to retry","Suppress the error and return an empty result so the agent doesn't stop","Log the error server-side only, without returning anything to the agent"],a:1,
...
e:"A, B, and C are independently-true Task 2.3 mechanisms: architecturally excluding write tools from the analysis agent's MCP server (A), restricting each agent's tool set to its role generally (B), and replacing an overly generic tool with constrained, purpose-specific alternatives when one would expose more capability than a role needs (C, the guide's fetch_url/load_document pattern applied here). D is the prompt-based approach this scenario's own architecture-level fix is chosen over — probabilistic, not a guarantee. E is real Task 2.3 content at the wrong problem: 'auto' controls whether a tool is called at all, not which tools are available to choose from. F is real, wrong problem: auditing is retrospective, it doesn't prevent access in the first place."},
...
e:"A and B are both Task 2.1 fixes and independent: A differentiates the two existing tools, B replaces them with purpose-specific tools (the guide's own extract_data_points / summarize_content / verify_claim_against_source pattern). Neither implies the other. C is a real pattern (consolidation) with flawed execution: a kitchen-sink bullet-point description reintroduces the exact lack-of-differentiation problem it's meant to solve. D misapplies Task 2.3's too-many-tools principle at the wrong scope — two tools is not an oversized set. E is real Task 2.3 content at the wrong scope: it guarantees a call, not the correct one."},
...
e:"A and B are independent Task 2.1 fixes: A is renaming to eliminate overlap, B is the boundary-explanation element of a good description — a clear name doesn't guarantee a stated boundary, and vice versa. C is a real engineering practice, wrong problem: schema reuse keeps parameters consistent, it doesn't touch tool-selection reasoning. D is real Task 2.3 content at the wrong scope: it forces list_orders even when a single order_id is already known, and doesn't teach the model to distinguish the tools on their merits. E misuses a real API parameter: max_tokens governs output length, it has no mechanism connecting it to tool-selection accuracy."},
...
{d:"Context Management & Reliability",q:"A subagent's tool call fails with a transient error partway through its task. The subagent immediately reports the failure up to the coordinator without attempting anything itself. What is the correct design?",o:["This is correct — subagents should always defer every error to the coordinator for a decision","The subagent should attempt local recovery for the transient failure itself (such as a retry), and only propagate the error to the coordinator if it cannot resolve it locally, including what was attempted and any partial results","The subagent should silently discard the failure and return whatever partial output it has as if it succeeded","The subagent should terminate itself and let the coordinator restart the entire workflow from the beginning"],a:1,
