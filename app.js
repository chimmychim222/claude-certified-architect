...
{d:"Claude Code Configuration",q:"You want to create a custom slash command /deploy that runs a specific deployment script. Where do you define this command?",o:["In the hooks block of .claude/settings.json, so a PreToolUse matcher runs the deployment script","In .claude/commands/deploy.md as a markdown file describing what the command should do","In a SKILL.md whose disable-model-invocation field puts the name in the slash menu","In a bash alias in ~/.bashrc"],a:1,
