...
{d:"Tool Design & MCP Integration",q:"Your MCP server exposes two tools, extract_summary and extract_report, with nearly identical one-line descriptions. The agent frequently calls the wrong one. What is the most effective fix?",o:["Rename one of the tools to something more distinctive without changing the descriptions","Expand both descriptions to clearly differentiate their purpose, expected inputs and outputs, and when to use one versus the other — or split them into more clearly scoped tools","Give both tools identical descriptions but add a 'priority' field so the agent picks the higher one","Remove one of the two tools so there is no ambiguity"],a:1,
e:"Ambiguous or overlapping tool descriptions are a primary cause of tool misrouting. The fix is to expand descriptions with clear boundaries — what each tool does, its inputs and outputs, and when to use it versus similar tools — or to split an overly generic tool into purpose-specific tools with defined contracts. Renaming alone doesn't resolve the underlying ambiguity, and an arbitrary priority field doesn't teach the model when each tool is actually appropriate."},
...
{d:"Context Management & Reliability",q:"Two credible sources your research agent consulted report different figures for the same statistic. The synthesis agent must combine these findings into a final report. What is the correct way to handle the conflict?",o:["Silently use the more recent source's figure and discard the other","Annotate the conflict, preserving both values with their source attribution, so the report notes that sources disagree rather than arbitrarily picking one","Average the two figures to produce a single value","Omit the statistic entirely since the sources disagree"],a:1,
e:"When credible sources disagree, the synthesis agent should annotate the conflict and preserve both values with source attribution rather than arbitrarily selecting or averaging them. This preserves the claim-source mapping and lets the coordinator or a human reviewer decide how to reconcile the discrepancy, instead of silently hiding the disagreement from the reader."},
