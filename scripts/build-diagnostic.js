...
/* ── EMAIL CAPTURE ── */
.email-capture{background:var(--surface2);border:1px solid var(--border);border-radius:12px;padding:22px 24px;text-align:center;margin-top:20px}
.email-capture-heading{font-family:-apple-system,system-ui,'Segoe UI',sans-serif;font-weight:600;font-size:.97rem;color:var(--text);margin:0 0 12px}
...
.email-status-ok{font-family:-apple-system,system-ui,'Segoe UI',sans-serif;font-size:.88rem;color:#059669;margin:10px 0 0}
.email-status-err{font-family:-apple-system,system-ui,'Segoe UI',sans-serif;font-size:.88rem;color:#dc2626;margin:10px 0 0}
...
function showResults() {
  ...
  lastDiagResults = {
    ...
    weakestDomainWeight: weakest.pct,
    domains: DOMAINS.map(d => ({
      label: d.label,
      examWeight: d.pct,
      correct: domScore[d.key].correct,
      total: domScore[d.key].total,
      pct: domScore[d.key].total > 0 ? Math.round((domScore[d.key].correct / domScore[d.key].total) * 100) : 0
    }))
  };

  // Reset email form in case user retook the quiz
  const ef = document.getElementById('email-form');
  const es = document.getElementById('email-status');
  if (ef) ef.style.display = '';
  if (es) { es.style.display = 'none'; es.textContent = ''; }
  const eb = document.getElementById('email-btn');
  if (eb) { eb.disabled = false; eb.textContent = 'Send results'; }
...
async function submitResultsEmail(e) {
  e.preventDefault();
